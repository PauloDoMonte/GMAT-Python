...
def criar_modelo():
    if os.path.exists(f'model_checkpoint_relu_and_tanh_{neuronios}.keras'):
        model = tf.keras.models.load_model(f'model_checkpoint_relu_and_tanh_{neuronios}.keras')
        ...
        model = keras.Sequential([
            ...
            layers.Dense(neuronios, activation='relu'),
            layers.Dropout(0.2),
        ...
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),
                      ...
                      metrics=['mape', 'mse', 'mae', r2_metric])
...
def treinar():
    ...
    checkpoint_callback = keras.callbacks.ModelCheckpoint(
        filepath=f'model_checkpoint_relu_and_tanh_{neuronios}.keras',
    ...
    early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', patience=100, min_delta=1e-8,
                                               restore_best_weights=True)
    reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', mode='min', factor=0.1, patience=10,
                                                    min_lr=1e-10, cooldown=10,
                                                    verbose=2)

    print("Inicializando o treinamento")
    history = model.fit(x_train, y_train, epochs=10000000, validation_data=(x_test, y_test), batch_size=batch_size,
                        callbacks=[reduce_lr, early_stop, current_epoch_callback, checkpoint_callback])
